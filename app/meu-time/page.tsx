...
export default function MeuTimePage() {
  ...
  const [placar, setPlacar] = useState<{ casa: number; visitante: number } | null>(null);
  ...
  useEffect(() => {
    ...
    fetchNextUpcomingMatch(clubId).then(async (nextMatch) => {
      ...
      if (lineup && lineup.placarCasa != null && lineup.placarVisitante != null) {
        setPlacar({ casa: lineup.placarCasa, visitante: lineup.placarVisitante });
      } else {
        setPlacar(null);
      }
      selected.sort(
  ...
  return (
    <div className="mx-auto max-w-6xl px-4 py-8">
      <div className="mb-8 flex flex-col gap-4">
        ...
        {match && placar != null && (
          <div className="rounded-lg border border-blue-200 bg-white px-4 py-2 shadow-sm">
            <p className="text-sm text-blue-600">Seu palpite de placar</p>
            <p className="text-lg font-bold text-blue-900">
              {HOME_TEAM} {placar.casa} x {placar.visitante} {match.opponent}
            </p>
          </div>
        )}
        {match && !isTeamEditLocked(match) && (
